...
class Users(Base):
    ...
    watch_history = relationship(
        "UserWatchHistory", backref=backref("user", lazy="joined"), cascade="all, delete-orphan"
    )
...
class UserWatchHistory(Base):
    ...
    trakt_uuid = Column(String, ForeignKey("users.trakt_uuid"), index=True, nullable=False)
    trakt_url = Column(String, ForeignKey("trakt_media.trakt_url"), index=True, nullable=False)
...
class TraktMedia(Base):
    ...
    show_imdb_id = Column(String, ForeignKey("imdb_media.show_imdb_id"), index=True, nullable=True)
    show_tmdb_id = Column(Integer, ForeignKey("tmdb_media.show_tmdb_id"), index=True, nullable=True)

    # One-to-Many relationships with UserWatchHistory
    watch_events = relationship(
        "UserWatchHistory", backref=backref("trakt_media", lazy="joined"), cascade="all, delete-orphan"
    )

    # Many-to-One relationships with IMDBMedia and TMDBMedia
    imdb_media = relationship("IMDBMedia", backref=backref("trakt_medias", lazy="dynamic"), lazy="joined")
    tmdb_media = relationship("TMDBMedia", backref=backref("trakt_medias", lazy="dynamic"), lazy="joined")
